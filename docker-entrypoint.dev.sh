...
# populated by npm *inside the Linux container*. If you run `npm install` on macOS, those
# binaries are for darwin — Tailwind/lightningcss will fail with missing .linux-*.node files.
