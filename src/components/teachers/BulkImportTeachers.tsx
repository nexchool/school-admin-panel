...
export function BulkImportTeachers({
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await teachersService.bulkImportPreview(fd);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await teachersService.bulkImport(fd);
      setImportResult(res);
      setStep("results");
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            (active or inactive).{" "}
            <code className="text-xs">department</code> must match an
            existing department&apos;s name exactly (case-insensitive) —
            manage the list under Academics → Departments. An unknown name
            fails just that row; it is never auto-created. Unknown columns
        ...
        {step === "preview" && (
          <div className="space-y-4">
            ...
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              ...
              />
              Send welcome email (in-app notification is always sent)
            </label>
